...
def deposit_scenarios(cfg: AccountConfig, amounts: list[float]) -> pd.DataFrame:
    ...
    for amount in amounts:
        frame = project_savings(cfg, amount, cfg.start_date + timedelta(days=3650))
        hit = first_goal_date(frame)
        if hit:
            through = frame[frame["date"] <= hit]
            days = (hit - cfg.start_date).days
            rows.append({"Deposit / Paycheck": f"${amount:,.0f}", "Goal Date": hit.strftime("%b %d, %Y"), "Months": round(days / 30.4375, 1), "Interest": f"${through['interest'].sum():,.2f}", "amount": amount, "months_raw": days / 30.4375})
    return pd.DataFrame(rows)
    ...
    while d < end_date:
        ...
        if d == next_deposit:
            if orsa < cfg.high_yield_limit:
                ...
                to_orsa, to_next = 0.0, deposit_amount
            orsa += to_orsa
            bucket2 += to_next
            next_deposit += timedelta(days=cfg.paycheck_frequency_days)
...
with plan_tab:
    ...
    max_date=min(TODAY+timedelta(days=365*5),hit+timedelta(days=180) if hit else TODAY+timedelta(days=365*5)); plot=frame[frame["date"]<=max_date].copy(); deposits=plot[plot["deposit"]>0]
    fig=go.Figure(); fig.add_trace(go.Scatter(x=plot["date"],y=plot["balance"],mode="lines",line=dict(color="rgba(24,200,255,.18)",width=13),hoverinfo="skip",showlegend=False)); fig.add_trace(go.Scatter(x=plot["date"],y=plot["balance"],mode="lines",name="Projected balance",line=dict(color="#18c8ff",width=4),fill="tozeroy",fillcolor="rgba(8,102,255,.17)")); fig.add_trace(go.Scatter(x=deposits["date"],y=deposits["balance"],mode="markers",name="Paycheck deposit",marker=dict(color="#ff7a00",size=8,line=dict(color="#ffd23f",width=1)))); fig.add_hline(y=1000,line_dash="dash",line_color="#ef233c",line_width=4,annotation_text="⚡ $1,000 PREMIUM ZONE FILLED",annotation_font_color="#ff6677"); fig.update_layout(height=470,margin=dict(l=5,r=5,t=30,b=5),paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="#030916",font_color="#eaf4ff",legend=dict(orientation="h",y=1.08),xaxis=dict(gridcolor="#122a48"),yaxis=dict(gridcolor="#122a48",tickprefix="$",rangemode="tozero"),hovermode="x unified"); st.plotly_chart(fig,use_container_width=True,config={"displayModeBar":False})
    gauge=go.Figure(go.Indicator(mode="gauge+number",value=min(starting_balance,1000),number={"prefix":"$","font":{"size":44,"color":"#fff"}},title={"text":"CURRENT PROGRESS TO $1,000","font":{"color":"#9fc7ff"}},gauge={"axis":{"range":[0,1000],"tickprefix":"$"},"bar":{"color":"#18c8ff","thickness":.32},"bgcolor":"#040b17","bordercolor":"#1f61b9","steps":[{"range":[0,500],"color":"#082958"},{"range":[500,800],"color":"#153b6c"},{"range":[800,1000],"color":"#55101c"}],"threshold":{"line":{"color":"#ef233c","width":6},"thickness":.85,"value":1000}})); gauge.update_layout(height=300,margin=dict(l=25,r=25,t=55,b=10),paper_bgcolor="rgba(0,0,0,0)",font_color="white"); st.plotly_chart(gauge,use_container_width=True,config={"displayModeBar":False})
with race_tab:
    ...
    amounts=sorted(set([5,10,15,20,25,30,40,50,75,100,125,150,int(deposit_amount)])); scenarios=deposit_scenarios(cfg,amounts); colors=["#0866ff" if a<25 else "#18c8ff" if a<50 else "#ff7a00" if a<100 else "#ef233c" for a in scenarios["amount"]]
    fig2=go.Figure(go.Bar(x=scenarios["amount"],y=scenarios["months_raw"],marker=dict(color=colors,line=dict(color="#fff",width=.4)),text=scenarios["Months"].map(lambda x:f"{x:.1f} mo"),textposition="outside")); fig2.update_layout(height=430,margin=dict(l=5,r=5,t=20,b=5),paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="#030916",font_color="#eaf4ff",xaxis_title="Deposit each paycheck ($)",yaxis_title="Months to $1,000",xaxis=dict(gridcolor="#122a48"),yaxis=dict(gridcolor="#122a48")); st.plotly_chart(fig2,use_container_width=True,config={"displayModeBar":False}); st.dataframe(scenarios[["Deposit / Paycheck","Goal Date","Months","Interest"]],hide_index=True,use_container_width=True)
with next_tab:
    st.markdown("<div class='section'>Keep Capitalizing After $1,000</div><div class='sub'>Fill the 10% zone, then redirect every future deposit instead of letting the habit stop.</div>",unsafe_allow_html=True)
    n1,n2,n3=st.columns(3)
    with n1: next_apy_pct=st.number_input("Next account APY (%)",0.0,20.0,4.50,.10)
    with n2: total_goal=st.number_input("Total cash-savings goal",1000.0,50000.0,5000.0,500.0)
    with n3: sweep=st.toggle("Sweep ORSA overflow",value=True)
    strategy=project_two_bucket(cfg,float(deposit_amount),next_apy_pct/100,float(total_goal),sweep,TODAY+timedelta(days=365*12)); orsa_rows=strategy[strategy["ORSA"]>=1000]; orsa_hit=None if orsa_rows.empty else orsa_rows.iloc[0]["date"]; goal_rows=strategy[strategy["Total"]>=float(total_goal)]; goal_hit=None if goal_rows.empty else goal_rows.iloc[0]["date"]
    ...
    st.markdown("<div class='next'><span class='step'>1</span><b>Fill the 10% ORSA zone.</b></div><div class='next'><span class='step'>2</span><b>Redirect new deposits immediately.</b></div><div class='next'><span class='step'>3</span><b>Grow the emergency fund without losing momentum.</b></div>",unsafe_allow_html=True)
    end_show=min(TODAY+timedelta(days=365*6),goal_hit+timedelta(days=120) if goal_hit else TODAY+timedelta(days=365*6)); sp=strategy[strategy["date"]<=end_show]; fig3=go.Figure(); fig3.add_trace(go.Scatter(x=sp["date"],y=sp["ORSA"],stackgroup="one",name="ORSA 10% bucket",line=dict(color="#0866ff",width=3),fillcolor="rgba(8,102,255,.50)")); fig3.add_trace(go.Scatter(x=sp["date"],y=sp["Bucket #2"],stackgroup="one",name="Next HYSA bucket",line=dict(color="#ef233c",width=3),fillcolor="rgba(239,35,60,.42)")); fig3.add_hline(y=float(total_goal),line_dash="dot",line_color="#ff7a00",line_width=3,annotation_text=f"🔥 ${total_goal:,.0f} TOTAL GOAL",annotation_font_color="#ff9f35"); fig3.update_layout(height=470,margin=dict(l=5,r=5,t=30,b=5),paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="#030916",font_color="#eaf4ff",legend=dict(orientation="h",y=1.08),xaxis=dict(gridcolor="#122a48"),yaxis=dict(gridcolor="#122a48",tickprefix="$"),hovermode="x unified"); st.plotly_chart(fig3,use_container_width=True,config={"displayModeBar":False}); st.markdown("<div class='note'><b>Strategy:</b> keep the automatic savings habit. Once the high-rate tier is full, change the destination — not the behavior.</div>",unsafe_allow_html=True)
with history_tab:
    ...
    if p.exists():
        ...
        if not div.empty:
            hfig=go.Figure(go.Bar(x=div["date"],y=div["amount"],marker=dict(color=["#0866ff" if i%3==0 else "#ff7a00" if i%3==1 else "#ef233c" for i in range(len(div))]),text=div["amount"].map(lambda x:f"${x:.2f}"),textposition="outside")); hfig.update_layout(height=330,margin=dict(l=5,r=5,t=20,b=5),paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="#030916",font_color="#eaf4ff",yaxis=dict(tickprefix="$",gridcolor="#122a48"),xaxis=dict(gridcolor="#122a48")); st.plotly_chart(hfig,use_container_width=True,config={"displayModeBar":False})
    else: st.info("Savings transaction file is not present yet.")
